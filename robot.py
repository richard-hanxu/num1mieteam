...
class Robot():
    ...
    def move(self, velocity, rotation, walls):
        ...
        # Create a deep copy of the old position
        old_position = pm.Vector2(self.position.x, self.position.y)
        ...
        # if old_position.x != self.position.x or old_position.y != self.position.y or old_rotation != self.rotation:
        #     print("Robot position:", self.position, "Rotation:", self.rotation)
    ...
    def teleport(self, x, y, angle, walls):
        ...
        # Check if each of the block corners are in a wall
        for corner in self.outline_global:
            if utilities.in_block(corner):
                self.position = original_position[0]
                self.rotation = original_position[1]
                self.update_outline()
                return False
        if not utilities.in_block(self.position) and not self.check_collision_walls(walls):
            return True
        else:
            self.position = original_position[0]
            self.rotation = original_position[1]
        ...
        for segment_bot in self.outline_global_segments:
            for square in walls:
                for segment_wall in square:
                    ...
                    print("Robot", segment_bot.x, segment_bot.y)
                    print("Wall", segment_wall.x, segment_wall.y)
    ...
    def command(self, cmds: list, environment: dict):
        ...
        if cmds[0] == 'tp' and len(cmds[1].split(",")) == 3:
            tp_state = cmds[1].split(",")
            x, y, angle = float(tp_state[0]), float(tp_state[1]), float(tp_state[2])
            success = self.teleport(x, y, angle, environment.get('walls', []))
            print("Success:", success)
            responses.append(['tp', 'Teleported' if success else 'Teleport Failed'])
            return responses
        
        for cmd in cmds:
            if cmd[0] == 'pp':
                # Print position command
                responses.append(['pp', f"{self.position.x:.{CONFIG.round_digits}f} {self.position.y:.{CONFIG.round_digits}f} {self.rotation:.{CONFIG.round_digits}f}"])
                continue
